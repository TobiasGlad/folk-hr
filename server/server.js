...
function normalizeState(state) {
  ...
  return {
    ...state,
    people: Array.isArray(state.people) ? state.people.map(person => {
    ...
    groups: normalizeGroups(Array.isArray(state.groups) && state.groups.length ? state.groups : seedState.groups),
    groupTypes: normalizeGroupTypes(Array.isArray(state.groupTypes) && state.groupTypes.length ? state.groupTypes : (legacyGroups.length ? legacyGroups.map(group => typeof group === 'string' ? group : group?.type || group?.groupType || '') : seedState.groupTypes)),
...
function readState() {
  ...
  try {
    const state = ensureSeedUsers(normalizeState(JSON.parse(row.state_json)));
    const hasBadInitialSeed = (!Array.isArray(state.people) || state.people.length === 0)
      && Array.isArray(state.groups)
      && state.groups.some(group => (typeof group === 'string' ? group : group?.name || group?.unit || '') === 'Orjanshuset' || (typeof group === 'string' ? group : group?.name || group?.unit || '') === 'Verksamhetsstod');
...
function writeState(state) {
  const next = ensureSeedUsers(normalizeState({
    people: Array.isArray(state.people) ? state.people : [],
    groups: Array.isArray(state.groups) ? state.groups : seedState.groups,
    groupTypes: Array.isArray(state.groupTypes) ? state.groupTypes : seedState.groupTypes,
    calendarEvents: Array.isArray(state.calendarEvents) ? state.calendarEvents : [],
    admins: Array.isArray(state.admins) ? state.admins : seedState.admins,
    retentionDays: Number(state.retentionDays) || seedState.retentionDays,
    colorTheme: ["folk", "mikaelgarden"].includes(state.colorTheme) ? state.colorTheme : seedState.colorTheme,
